...
import { FaChartPie, FaBoxOpen, FaTrashAlt, FaQuestionCircle, FaRobot, FaSpinner, FaUtensils } from 'react-icons/fa';
...
const RecipeAssistant = () => {
  ...
  const generateAIAnalysis = async () => {
    setIsLoading(true);
    try {
        const stockSummary = inventory.map(p => `${p.name}: ${p.quantity}`).join('\n');
  ...
  return (
    ...
    >
      ...
      <div className="max-w-4xl mx-auto mb-8">
        <h1 className="text-4xl font-bold text-gray-100 mb-2">Recipe Assistant</h1>
        <p className="text-gray-400">Get personalized recipe suggestions based on your inventory</p>
      </div>
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        >
          <div className="flex items-center space-x-3 mb-4">
            ...
            <h2 className="text-xl font-semibold text-purple-300">Available Ingredients</h2>
          </div>
            ...
            {inventory.map((item, index) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-purple-400 font-medium">{item.quantity}</span>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-3">
              ...
              <h2 className="text-xl font-semibold text-blue-300">Recipe Suggestions</h2>
            </div>
            <motion.button
          ...
          {inventory.length === 0 && (
            <div className="bg-yellow-900/20 border border-yellow-700/30 rounded-lg p-4 mb-4">
              <p className="text-yellow-300 text-sm">Please add ingredients to your inventory to get recipe suggestions.</p>
            </div>
          )}
          ...
          {aiAnalysis && (
            ...
            >
              <pre className="whitespace-pre-wrap text-gray-300 text-sm font-medium leading-relaxed">
                {aiAnalysis}
              </pre>
            </motion.div>
          )}
